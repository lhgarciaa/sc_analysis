...
def draw_angle_lines(ara_level, use_custom_centers=False):
    ara_level = str(ara_level)
    y_center, x_center = get_sc_circle_centers(ara_level, use_custom_centers=use_custom_centers)
...
def main():
    ...
    for sc_level in sc_levels:
        ...
        cv2.imwrite('sc_angle_lines/sc_angle_lines_ara_{}.tif'.format(str(sc_level).zfill(3)), angle_lines)
