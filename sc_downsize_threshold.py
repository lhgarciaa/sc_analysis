...
from sc_util import sc_levels, sc_custom_atlas_levels, threshold_image_generator


def get_custom_wireframe(ara_level):
    if ara_level in sc_custom_atlas_levels:
        wireframe_path = 'sc_wires/SC/v3/{}_SC_rgb_atlas_wire_with_angle_lines.tif'.format(str(ara_level).zfill(3))
    else:
        wireframe_path = 'sc_wires/ARA/ARA-Coronal-{}_full_labels-01-append_wire_with_angle_lines.tif'.format(str(ara_level).zfill(3))
    return cv2.imread(wireframe_path, cv2.IMREAD_GRAYSCALE)


def make_downsize_threshold(sc_level):
    df = get_clean_df()
    wireframe = get_custom_wireframe(sc_level)
    tracer_rgb_map = {
    ...
    for case_id, case_slide, channel, threshold in threshold_image_generator(sc_level, apply_sc_mask=False, return_case_slide=True):
        ...
        output_dir = os.path.join('sc_threshold_downsize', case_id, 'threshold', 'channels', channel)
